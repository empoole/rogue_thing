...
class CharacterScreenEventHandler(AskUserEventHandler):
	...
	def on_render(self, console: tcod.console.Console) -> None:
		...
		x = 40 if engine.player.x <= 30 else 0
		y = 0
		...
		console.print(
			x=x + 1,
			y=y + 2,
			string=f"XP: {self.engine.player.level.current_xp} / {self.engine.player.level.xp_to_next_level}",
		)
		console.print(
...
class LevelUpEventHandler(AskUserEventHandler):
	...
	def on_render(self, console: tcod.console.Console) -> None:
		...
		console.print(x=x + 1, y=2, string="Select and attribute to increase")
		...
		console.print(
			x=x + 1,
			y=5,
			string=f"b) Stregth (+1 attack, from {self.engine.player.figher.power})"
		)
		console.print(
			x=x + 1,
			y=6,
			string=f"c) Agility (+1 defense, from {self.engine.player.figher.defense})"
		)
	...
	def ev_mousebuttondown(
		...
		"""
		Don't allow player to accidentally click out of levelling up
		"""
		return None
